    ...
    public ResponseEntity<Product> update(@PathVariable Long id, @RequestBody ProductUpdateRequest updated) {
        return productService.findById(id)
                .map(existing -> {
                    ...
                    existing.setPrezzoBase(updated.getPrezzoBase());
                    existing.setAumentoPercentuale(updated.getAumentoPercentuale());

                    if (updated.getCategoriaId() != null) {
                        Category category = categoryRepository.findById(updated.getCategoriaId())
                                .orElse(null);
                        existing.setCategoria(category);
                    } else {
